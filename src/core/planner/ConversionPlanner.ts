  ...
  plan(sourceId:string,targetId:string):ConversionRoute {
    ...
    if (sourceId===targetId) {
      const direct=this.graph.outgoing(sourceId)
        ...
        .sort((a,b)=>edgeScore(a)-edgeScore(b))[0];
      if (!direct) throw new Error("No local conversion route is available on this browser.");
      return {edges:[direct],score:edgeScore(direct),warnings:analyzeLoss(source,target,[direct])};
    }
    ...
    while (queue.length) {
      ...
      if (current.id===targetId) {
        return {edges:current.edges,score:current.score,warnings:analyzeLoss(source,target,current.edges)};
      }
